...
func (cfg *Cfg) readWideSkyTheme() {
	...
	cfg.WideSkyTheme = &WideSkyThemeSettings{
		...
		BasedOff:    themeSec.Key("based_off").MustString("dark"),
		BaseColor:   baseColor,
		BorderColor: themeSec.Key("border_colour").MustString(baseColor),

		FontHeader: themeSec.Key("font_header").MustString(NOT_SET),
		FontBody:   themeSec.Key("font_body").MustString(NOT_SET),
